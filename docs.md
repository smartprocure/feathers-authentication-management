...
- expireOnFailedAttempt: Expire reset token if user tries to reset with invalid token. Default is true.
- reuseResetToken: Use the same reset token if user resets password twice. Default is false.
